...
Status: **accepted** (grilled 2026-09-13). Implementation via parent issue and do-work children after docs land.
...
| Existing data | N/A | Orphan Decks/Cards assigned to bootstrap admin on first User create |
...
- Migration: add `user_id` (and related FKs); on bootstrap admin create, attach rows with null/orphan ownership to that admin.

## Out of scope (v1.3)

- Courses, subscribe, Card copies / source links / version diffs (future grill)
- Roles beyond the single bootstrap admin (no promote UI; no create-course role grill yet)
...
2. Existing pre-v1.3 DB: after bootstrap admin is created, prior Decks/Cards belong to that admin and remain studyable.
